setwd("~/work/yachie/YUSUKE_KIJIMA.LAB/Experiments/Projects/droParser/tool_test/paper_data/spatial//")
library(dplyr)
library(Matrix)
library(Seurat)
library(ggplot2)
...
#     input = paste0("DGE/ed0/slide_",i,"_ed0_visium_Feb2021.edit.csv.gz"),
#     sep = ",",
#     data.table = FALSE
#   )
...
coord_aligned_10 <- read.table("../../slide_to_visium/20200507_bc_expansion/slide_to_vis_exp10_aligned.tsv",sep="\t",row.names = 1,header = T)
...
coord_aligned_10 <- read.table("../../slide_to_visium/cereb1/cereb1_exp10_aligned.tsv",sep="\t",row.names = 1,header = T)
df.grid  <- get_grid(coord_aligned_10)
...
for(i in 21:40){
  ...
  g1 <- ggplot(slide_loc_new,aes(x=x,y=y,col=clusters))+
    ...
    guides(colour=guide_legend(override.aes = list(alpha=1,size=3)))+
    NoLegend()
  ggsave(paste0("plot_210804/seed",i,"_cereb1_ed0_spatial.jpeg"),g1,width = 10,height = 10,units = "cm",dpi=600)
}
...
coord_aligned_10 <- read.table("../../slide_to_visium/cereb2/cereb2_exp10_aligned.tsv",sep="\t",row.names = 1,header = T)
df.grid  <- get_grid(coord_aligned_10)
...
for(i in 1:20){
  ...
  g1 <- ggplot(slide_loc_new,aes(x=x,y=y,col=clusters))+
    ...
    guides(colour=guide_legend(override.aes = list(alpha=1,size=3)))+
    NoLegend()
  ggsave(paste0("plot_210804/seed",i,"_cereb2_ed0_spatial.jpeg"),g1,width = 10,height = 10,units = "cm",dpi=600)
}
...
coord_aligned_10 <- read.table("../../slide_to_visium/liver/liver_exp10_aligned.tsv",sep="\t",row.names = 1,header = T)
df.grid  <- get_grid(coord_aligned_10)
...
for(i in 21:40){
  ...
  g1 <- ggplot(slide_loc_new,aes(x=x,y=y,col=clusters))+
    ...
    guides(colour=guide_legend(override.aes = list(alpha=1,size=3)))+
    NoLegend()
  ggsave(paste0("plot_210804/seed",i,".jpeg"),g1,width = 10,height = 10,units = "cm",dpi=600)
}
...
coord_aligned_10 <- read.table("../../slide_to_visium/kidney/kidney_exp10_aligned.tsv",sep="\t",row.names = 1,header = T)
df.grid  <- get_grid(coord_aligned_10)
